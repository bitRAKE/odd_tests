...
// clang -O3 -march=native -mavx512f -masm=intel -std=c23 testing.c vpowips.S -lgdi32 -luser32 -lkernel32 -lbcrypt -o testing.exe
//
...
#include "vpowips_shim.h" // __m512 vpowips_call(__m512 base, uint64_t exp)
...
static __m512 ref_vpowips(__m512 base, uint64_t exp);
...
static void init_console(void) {
    ...
    if (g_console != INVALID_HANDLE_VALUE) {
        SetConsoleTitleW(L"vpowips Test Harness");
    }
}
...
static BOOL execute_case(const char *group, const char *label, __m512 base, uint64_t exponent, functional_stats *stats) {
    __m512 expected = ref_vpowips(base, exponent);
    __m512 actual = vpowips_call(base, exponent);
...
static void run_performance_monitor(app_state *state) {
    ...
    for (uint32_t i = 0; i < warmup_iterations && state->running; ++i) {
        base = vpowips_call(base, (uint64_t)(i | 1u));
    }
...
//            }
            ...
            for (uint32_t iter = 0; iter < sample_iterations; ++iter) {
                acc = vpowips_call(base_vec, exponent);
            }
...
static DWORD WINAPI worker_thread_proc(LPVOID param) {
    ...
    if (kernel) {
        ...
        if (set_description) {
            set_description(GetCurrentThread(), L"vpowips worker");
        }
    }
...
static ATOM register_window_class(HINSTANCE instance) {
    WNDCLASSEXW wc = {
        ...
        .lpszMenuName = NULL,
        .lpszClassName = L"vpowipsWindowClass",
...
static HWND create_main_window(HINSTANCE instance, app_state *state) {
    ...
    HWND hwnd = CreateWindowExW(
        WS_EX_APPWINDOW,
        L"vpowipsWindowClass",
        L"vpowips Performance Monitor",
        WS_OVERLAPPEDWINDOW,
